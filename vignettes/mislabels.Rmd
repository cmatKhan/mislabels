...
I will address shortly, make VCF files from the output of Samtools pileup, also 
...
First, the filtering. The authors suggest stringent filters on the RNA sequence 
...
that they are unnecessarily stringent, and thus reduces the number of bases from 
which we may infer the sample's genotype. For instance, for counting reads 
over a gene, it is reasonable to remove reads which multi-map with equal confidence. 
However, for this purpose, if the read aligns perfectly to two or more positions, 
it should be used at both positions to add counts to a given base. We aren't 
comparing how many counts a given base has in this application -- we just need 
to know about what base occurs at a given position in a given sample, and 
well aligned multi-maps do tell us that. Similarly, the bioinformatics community 
has largely decided (at least in the RNA seq world) that you should not remove 
inferred duplicates (too hard to tell what is a PCR duplicate and what is a 
biological duplicate). Like multi-maps, if these possible duplicates align well, 
we should use that information. Finally, and there are more details on this 
...
from RNA sequence libraries. But, their suggested filters are wrong.
...
as opposed to extract data from Variant Call Files (VCF) match to data from BAM 
files. I wish to use VCF files, since they a) already exist and b) possess 
quality metrics on each SNP site already. Consider the VCF files as the result 
of thousands of BAM files -- I believe that using this method on a problem 
of this scale would take quite some time, but by using the VCF files, I reduce 
the data processing up front, since it is already done.  

This paper is a significant improvement 
on my method, though, as in the supplement it does detail the 
...
resources it would take to do a reasonable comparison.  
...
paper. But, in terms of both scaling and usability, surprisingly, this is not a 
bad implementation.
...
to 'pileup' the alignments from the BAM files. 

### Sample preparation

The genome SNP variants are sampled from 4000 highly and consistently expressed loci. 
to reduce the time for each distance calculation, rather than using all 4000, 
I created 4 samples of 1,900 genes drawn from the 4,000 genes. I then intersected 
those samples with the genome SNP database. The result of this is a list of SNPS 
which occur in the sampled regions. For sample1, for example, there are 840 high 
confidence SNPS. 
...
> sample_genome_vectors$`8`
 ...
 ...
```
Using the exactly bp coordinates of these SNPs, I then extract the consensus 
...
gene regions. Using my filters, which counts bases in reads in all good alignments, 
regardless of whether or not the read is identified as a multimap, significantly 
increases the number of "certain genotypes". The average number in any given SNP 
...
two scoring matrices below. Strict only accepts exact matches where all of the homozygous 
...
genomes exactly in at least one of the 4 SNP vector samples (Figure 3A). I 
am not sure what to expect, but this seems incredibly high, even for family 
data. There are samples included in this data set which are not actually part 
of the study, i.e. family members who didn't submit to the 3 visits and numerous 
measurements, but did allow their genome to be assembled to provide full pedigree 
data. Possibly this set, which I used to construct the SNP database, also includes 
some control samples.

Outside of those 38 exact matches, Figure 3A shows the first percentile, median, 
mean, third percentile and maximum number of differences between genome SNP 
...
```{r, eval = TRUE, fig.height=12, fig.width=5,  fig.cap="Figure 4: RNA seq vs all genomes results"}
library(patchwork)
